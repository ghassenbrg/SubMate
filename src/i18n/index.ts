const ENGLISH_MESSAGES = {
  extensionName: 'SubMate: Subtitle Translator',
  extensionDescription: 'Translate supported Netflix, TVer, and Prime Video text subtitles in Chrome.',
  tagline: "Translate what streaming doesn't.",
  onboardingDescription: "Translate subtitles your streaming service doesn't provide.",
  netflixDetected: 'Netflix episode detected',
  enableSubMate: 'Enable SubMate',
  translateTo: 'Translate to',
  targetLanguage: 'Target language',
  selectedLanguage: 'Selected language: $1',
  chooseLanguage: 'Choose a language',
  customLanguage: 'Other language or regional variant…',
  customLanguagePlaceholder: 'Enter a language code, e.g. es-MX',
  automaticLanguage: 'Automatic — match the active subtitle',
  invalidLanguage: 'Choose a language by name, or enter a valid language code.',
  subtitleDisplay: 'Subtitle display',
  originalAndTranslation: 'Original + Translation',
  translationOnly: 'Translation only',
  off: 'Off',
  privacySummary: 'On-device translation is private and free. No account or SubMate server is required.',
  getStarted: 'Get started',
  next: 'Next',
  translation: 'Translation',
  onDevice: 'On-device',
  manualTranslation: 'Manual translation',
  display: 'Display',
  sourceSubtitleToTarget: 'Source subtitle → $1',
  subtitleLines: '$1 subtitle lines',
  progressLines: '$1 / $2 lines',
  startTranslation: 'Start translation',
  retry: 'Retry',
  exportFormat: 'Export format',
  exportSubtitles: 'Export subtitles',
  importTranslation: 'Import translation',
  sourceExported: 'Source subtitles exported.',
  noActiveSubtitle: 'No active subtitle',
  fileTooLarge: 'The selected file is larger than 10 MB.',
  importMatched: '$1 $2 / $3 lines.',
  advancedSettings: 'Advanced settings →',
  tabScopeHint: 'Changes here apply to this tab and to tabs you open next.',
  starOnGitHub: 'Give us a ★ on ',
  starAria: 'Give SubMate a star on GitHub',
  notNetflix: 'Open a Netflix video to translate available subtitles.',
  noPlayer: 'Netflix detected. Start a movie or episode to use translated subtitles.',
  importReady: 'Translation matched and is ready to use.',
  importFailed: 'The translation could not be imported.',
  statusWaiting: 'Waiting for an episode.',
  statusDisabled: 'SubMate is off.',
  statusDiscovering: 'Finding available subtitles…',
  statusDownloadingSource: 'Downloading source subtitles…',
  statusParsingSource: 'Reading source subtitles…',
  statusCheckingCache: 'Checking saved translations…',
  statusTargetAvailable: 'This service already provides your selected target language.',
  chooseAnotherLanguage: 'Choose another language',
  statusNeedsActivation: 'Chrome needs a user action before starting the local translator.',
  statusDownloadingModel: 'Downloading language data…',
  statusTranslating: 'Preparing translated subtitles…',
  statusTranslatingInAnotherTab: 'Translating in another tab…',
  statusValidating: 'Checking the completed translation…',
  statusReady: 'Translated subtitles ready',
  statusSavedReady: 'Saved translation ready',
  statusImportedReady: 'Imported translation ready',
  statusImageUnsupported: 'This subtitle track is image-based. Automatic translation is not supported yet.',
  statusNoTextTrack: 'No suitable text subtitle was found for this episode.',
  statusFailed: "Translated subtitles couldn't be prepared. Playback can continue normally.",
  settingsTitle: 'SubMate Settings',
  settingsSubtitle: 'Private, synchronized subtitle translation for Netflix, TVer and Prime Video',
  general: 'General',
  autoTranslateEpisodes: 'Auto-translate episodes',
  targetLanguageHelp: 'Choose by full name. The standard code appears in parentheses; custom BCP-47 tags are also accepted.',
  preferredSourceLanguage: 'Preferred source language',
  sourceLanguageHelp: 'Optional. Leave automatic to match the active or audio subtitle.',
  engine: 'Engine',
  engineHelp: 'On-device subtitle text stays in Chrome. Manual mode uses export/import.',
  appearance: 'Appearance',
  subtitleStylePreset: 'Subtitle style',
  appearancePresetHelp: 'Start with a preset, then adjust any detail. Changes appear in the player immediately.',
  netflixStyle: 'Netflix-like',
  softBoxStyle: 'Soft background',
  solidBoxStyle: 'Solid black background',
  outlineStyle: 'Outline',
  minimalStyle: 'Minimal',
  customStyle: 'Custom',
  subtitleBackground: 'Subtitle background',
  backgroundNone: 'None',
  backgroundSoft: 'Translucent black',
  backgroundSolid: 'Solid black',
  textOutline: 'Text outline',
  outlineNone: 'None',
  outlineShadow: 'Soft shadow',
  outlineStrong: 'Strong outline',
  textColor: 'Text color',
  fontWeight: 'Text weight',
  subtitleOpacity: 'Opacity',
  lineSpacing: 'Line spacing',
  stylePreview: 'Preview',
  previewOriginal: 'Original subtitle',
  previewTranslation: 'Translated subtitle',
  displayMode: 'Display mode',
  fontSize: 'Font size',
  verticalPosition: 'Vertical position',
  showPlayerStatus: 'Show player status',
  fineTuneStyle: 'Fine-tune style',
  appearanceOffNote: 'Subtitles are turned off. Choose a display mode above to adjust how they look.',
  resetAppearance: 'Reset appearance',
  storage: 'Storage',
  cachedItems: '$1 cached translations · $2 source subtitles',
  extensionStorageUsed: 'Extension storage used: $1',
  clearCache: 'Clear downloaded subtitle data',
  clearCacheConfirm: 'Clear $1 cached translations and downloaded subtitle tracks? Your settings will not change.',
  about: 'About',
  version: 'Version $1',
  independentNotice: 'SubMate is independent and is not affiliated with or endorsed by Netflix, TVer or Amazon.',
  noTelemetryNotice: 'No telemetry or SubMate backend. On-device translation uses Chrome browser-managed language data.',
  developerDiagnostics: 'Developer diagnostics',
  diagnosticsHint: 'Open a supported episode to view diagnostics.',
  overlayWaiting: 'Waiting for a video',
  overlayFinding: 'Finding subtitles…',
  overlayReading: 'Reading source subtitles…',
  overlayChecking: 'Checking saved translations…',
  overlayNativeAvailable: 'Your target language is already available',
  overlayReadyToTranslate: 'Ready to translate',
  overlayPreparing: 'Preparing translated subtitles…',
  overlayImageUnsupported: 'Image-based subtitles are not supported yet',
  overlayNoText: 'No suitable text subtitle found',
  overlayFailed: "Translated subtitles couldn't be prepared",
  openQuickControls: 'Open SubMate controls',
  quickControls: 'SubMate quick controls',
  subtitleDisplayMode: 'Subtitle display mode',
  turnOff: 'Pause SubMate',
  turnOn: 'Resume SubMate',
  openSettings: 'Open settings',
  translationProgress: 'Translation progress',
  errorDownload: 'The source subtitle could not be downloaded. Playback can continue normally.',
  errorParse: 'The source subtitle format could not be read.',
  errorTranslatorUnavailable: 'On-device translation is unavailable in this browser.',
  errorPairUnsupported: 'On-device translation does not support this language pair.',
  errorLanguageData: 'Chrome could not prepare the language data.',
  errorWrongTrack: 'This translation was created for a different subtitle track.',
  errorAmbiguousImport: 'This subtitle file could not be matched safely to the current episode.',
  errorInvalidImport: 'This file could not be read as a SubMate translation.',
  errorPlayerState: 'The player is not ready for translated subtitles yet.',
  errorIncompleteTranslation: "This file doesn't cover enough of the episode's lines. Check that every subtitle line has a translation.",
  errorTranslationMismatch: "This file's lines don't line up with the current subtitle track.",
  importUntranslated: '$1 lines had no translation and will stay blank.',
  cloudTranslation: 'Cloud API',
  cloudSection: 'Cloud translation',
  cloudVendorLabel: 'Provider',
  cloudApiKey: 'API key',
  cloudApiKeyHelp: 'Stored on this device only and never synced. Leave blank and save to remove it.',
  cloudApiKeyPlaceholder: 'Paste your API key',
  cloudApiKeySaved: 'A key is saved on this device.',
  cloudApiKeyMissing: 'No key saved yet.',
  cloudApiKeyClear: 'Remove key',
  cloudModel: 'Model',
  cloudModelHelp: 'Leave blank to use the provider default.',
  cloudPrivacyWarning: 'Cloud translation sends subtitle text to your chosen provider. It leaves this device and is subject to that provider’s terms and pricing. On-device translation stays local.',
  errorCloudAuth: 'Your API key was rejected. Check it in settings.',
  errorCloudModel: "The model or endpoint wasn't found. Check them in settings.",
  errorCloudQuota: "Your cloud provider's rate limit or quota was reached. Try again later.",
  cloudProviderCustom: 'Custom (OpenAI-compatible)',
  cloudBaseUrl: 'Base URL',
  cloudBaseUrlHelp: 'Any OpenAI-compatible endpoint, such as Ollama, LM Studio, vLLM or LiteLLM. HTTPS, or HTTP on this computer.',
  cloudBaseUrlInvalid: 'Enter an HTTPS URL, or an HTTP URL on localhost.',
  cloudApiKeyOptional: 'Optional for local servers. Stored on this device only and never synced.',
  cloudGetKey: 'Get an API key from $1',
  cloudKeyDetected: 'Key recognized as $1, so the provider was switched.',
  cloudModelRequired: 'Enter the model name your server uses.',
  cloudPermissionNeeded: 'SubMate needs permission to reach $1.',
  cloudPermissionGrant: 'Allow access',
  errorCloudPermission: 'SubMate needs permission to reach your cloud provider. Allow it in settings.',
  errorCloudUnreachable: "Couldn't reach the cloud provider. Check your connection or the base URL.",
  errorCloudUntranslated: 'The model sent the subtitles back untranslated. Try a different model in settings.',
  statusCloudNotConfigured: 'Finish setting up cloud translation in settings.',
  statusAdPlaying: 'Advertisement playing — translated subtitles are paused.',
  overlayAdPlaying: 'Ad playing',
  platformDetected: '$1 episode detected',
  notSupportedSite: 'Open a supported video (Netflix, TVer or Prime Video) to translate available subtitles.',
  noPlayerGeneric: 'Supported site detected. Start a video to use translated subtitles.',
  themeToggleAria: 'Appearance: $1. Click to change.',
  themeSystem: 'Match system',
  themeLight: 'Light',
  themeDark: 'Dark',
  displayModeBoth: 'Both',
  displayModeTranslation: 'Translation',
  subtitleFiles: 'Subtitle files',
  exportAction: 'Export',
  importAction: 'Import',
  decreaseSetting: 'Decrease $1',
  increaseSetting: 'Increase $1',
} as const;

export type MessageKey = keyof typeof ENGLISH_MESSAGES;

export function t(key: MessageKey, substitutions: string | string[] = []): string {
  const values = typeof substitutions === 'string' ? [substitutions] : substitutions;
  try {
    const localized = chrome.i18n?.getMessage?.(key, values);
    if (localized) return localized;
  } catch {
    // Unit tests and non-extension documents use the English fallback.
  }
  return values.reduce(
    (message, value, index) => message.replaceAll(`$${index + 1}`, value),
    ENGLISH_MESSAGES[key] as string,
  );
}

export function uiLocale(): string {
  try { return chrome.i18n?.getUILanguage?.() || navigator.language || 'en'; }
  catch { return 'en'; }
}

export function isRtlLocale(locale = uiLocale()): boolean {
  return ['ar', 'fa', 'he', 'ur'].includes(locale.toLowerCase().split('-')[0] ?? '');
}

export function applyDocumentLocale(): void {
  const locale = uiLocale();
  document.documentElement.lang = locale;
  document.documentElement.dir = isRtlLocale(locale) ? 'rtl' : 'ltr';
  document.title = t(document.body.dataset.page === 'options' ? 'settingsTitle' : 'extensionName');
}
